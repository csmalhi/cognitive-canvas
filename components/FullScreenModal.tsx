...
const typeIconMap = {
    image: <ImageIcon className="w-16 h-16 text-brand-blue" />,
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {result.type === 'image' && !isDriveFile ? (
            <img src={result.url} alt={result.title} className="max-w-full max-h-[70vh] mx-auto object-contain" />
          ) : (
             <div className="flex flex-col items-center justify-center h-full text-center text-gray-400">
                {result.type === 'image' && isDriveFile && (
                    <img src={result.url} alt={result.title} className="max-w-full max-h-[50vh] mx-auto object-contain mb-4 rounded-lg" />
                )}
                <div className="mb-4">{typeIconMap[result.type]}</div>
                <h3 className="text-lg text-white mb-2">{result.title}</h3>
                <p className="max-w-md mb-4">{result.description}</p>
                {result.content && !isDriveFile && (
                ...
                {isDriveFile && (
                    <a 
                        ...
                        className="inline-flex items-center gap-2 bg-brand-blue text-white font-semibold px-4 py-2 rounded-lg hover:bg-brand-blue/80 transition-colors mt-4"
                    >
